...
export class IndicadorDTO {
    ...
    public constuirDTO(nombre: String, valor: number, tipo: string, limiteInferior?: number, limiteSuperior?: number) { // Constructor para construir un objeto DTO basado en un Objeto Entity
        this.nombre = nombre
        this.valor = valor
        this.tipo = tipo
        if (limiteInferior)
