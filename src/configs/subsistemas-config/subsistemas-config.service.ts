...
export class SubsistemasConfigService {
  constructor(
    @InjectRepository(SubsistemaConfig)
    private subSistemaConfigRepository: Repository<SubsistemaConfig>,
  ...
  public async getAllSubsistemasConfig(
    ...
    nombre?: String,
  ): Promise<ApiPaginatedResponse<SubsistemaConfigSerializable[]>> {
    const subsistemasConfigSerializable: Array<SubsistemaConfigSerializable> =
      new Array<SubsistemaConfigSerializable>();
    // se obtienen los subsistemas config de la base de datos
    const subsistemasConfig: Array<SubsistemaConfig> =
      await this.subSistemaConfigRepository.find({
        where: {
    ...
    for (let index = 0; index < subsistemasConfig.length; index++) {
      ...
      subsistemasConfigSerializable.push(
        new SubsistemaConfigSerializable(
          subsistemaConfig.id,
          subsistemaConfig.nombre,
          await subsistemaConfig.cantMateriales(),
        ),
      );
    }

    return { data: subsistemasConfigSerializable };
  }
  ...
  public async createSubSistemaConfig(
    subSistemaConfigDTO: SubsistemaConfigDTO,
    entityManager?: EntityManager,
  ) {
    // si no existe un subsistema con ese nombre
    if (
      !(await this.getSubSistemaConfig(
        undefined,
        subSistemaConfigDTO.sistemaConfig.id,
        subSistemaConfigDTO.nombre,
      ))
    ) {
      if (!entityManager)
        // No se trata de una llamada con una transacción heredada
        await this.subSistemaConfigRepository.manager.transaction(
          async (transactionManager: EntityManager) => {
            // Se crea una transaccion para este procedimiento
            await this.createSubSistemaConfigWithEntity(
              subSistemaConfigDTO,
              transactionManager,
            );
          },
  ...
  private async createSubSistemaConfigWithEntity(
    subSistemaConfigDTO: SubsistemaConfigDTO,
    entityManager: EntityManager,
  ) {
    const subSistemaConfig: SubsistemaConfig = new SubsistemaConfig(
      undefined,
      subSistemaConfigDTO.nombre,
      subSistemaConfigDTO.sistemaConfig instanceof SistemaConfig
        ? subSistemaConfigDTO.sistemaConfig
        : new SistemaConfig(subSistemaConfigDTO.sistemaConfig.id),
    ); // se crea el subSistemaConfig para ser insertado

    const subSistemaConfigInsertado: SubsistemaConfig =
      await entityManager.save(subSistemaConfig); // se inserta en la base de datos el subSistemaConfig y se obtiene una instancia con el id insertado

    if (subSistemaConfigDTO.materialesConfig)
      await this.saveMaterialesConfig(
        subSistemaConfigDTO.materialesConfig,
        subSistemaConfigInsertado,
        entityManager,
      ); // se inserta en la base de datos todos los materialesConfig pertenecientes al subSistemaConfig
  }

  // Metodo para insertar en la base de datos todos los materiales config pertenecientes al subSistemaConfig
  private async saveMaterialesConfig(
    materialesConfigDTO: Array<MaterialConfigDTO>,
    subSistemaConfigInsertado: SubsistemaConfig,
    entityManager: EntityManager,
  ) {
    ...
    for (let index = 0; index < materialesConfigDTO.length; index++) {
      ...
      await this.materialConfigService.createMaterialConfig(
        materialesConfigDTO[index],
        entityManager,
      ); // Se manda a insertar al servicio de materialConfig en la base de datos
    }
  }
  ...
  public async deleteSubsistemaConfig(idSubsistemaConfig: number) {
    ...
    const subSistemaConfigEliminar: SubsistemaConfig | undefined =
      await this.subSistemaConfigRepository.findOne({
        where: {
          id: idSubsistemaConfig,
        },
    ...
    if (subSistemaConfigEliminar) {
      ...
      await this.eventEmitter.emitAsync(
        'accionCritica',
        subSistemaConfigEliminar.sistemaConfig.configVersion,
      );
    } else
      throw new HttpException(
        'No existe un Subsistema Config con ese id',
        HttpStatus.BAD_REQUEST,
      );
  }
}
