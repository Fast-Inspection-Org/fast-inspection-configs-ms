...
export class HerramientaAnalisisCriticidadService {
    ...
    constructor(@InjectRepository(HerramientaAnalisisCriticidad) private herramientaAnalisisCriticidadRepository: Repository<HerramientaAnalisisCriticidad>,
        private campoService: CampoService, private herramientaService: HerramientasService) { }

    // Método para obtener una herramienta análisis criticidad
    public async getHerramientaAnalisisCriticdad(idHerramienta?: number, nombre?: string) {
    ...
    public async createHerramientaAnalisisCriticidad(herramientaAnalisisCriticidadDTO: HerramientaDTO, entityManager?: EntityManager) {
        if (!(await this.herramientaService.getHerramienta(herramientaAnalisisCriticidadDTO.nombre, undefined, herramientaAnalisisCriticidadDTO.config.version))) { // si no existe esa herrramienta en la base de datos
    ...
    public async getAllHerrramientasAnalisisCriticidad(versionConfig?: number, nombre?: String) {
    ...
    public async updateHerramientaAnalisisCriticidad(idHerramienta: number, updateHerramientaAnalisisCriticidad: UpdateHerramientaAnalisisCriticidadDTO) {
        // se obtiene la herramienta análisis de criticidad a modificar
        const herramientaAnalisisCricidadModificar: HerramientaAnalisisCriticidad = await this.getHerramientaAnalisisCriticdad(idHerramienta)

        // se comprueba que no exista una herramienta con el mismo nombre
        if (!(await this.herramientaService.getHerramienta(updateHerramientaAnalisisCriticidad.nombre, undefined, herramientaAnalisisCricidadModificar.configVersion))) {
            herramientaAnalisisCricidadModificar.nombre = updateHerramientaAnalisisCriticidad.nombre // se modifica el nombre de la herramienta análisis de criticidad
            this.herramientaAnalisisCriticidadRepository.save(herramientaAnalisisCricidadModificar) // se guarda la herramienta con los cambios
        }
        else
            throw new HttpException("Ya existe una herramienta con ese nombre en la base de datos", HttpStatus.BAD_REQUEST);
    }
