...
import {
  CampoDefinido,
  TiposCamposDefinidos,
} from '../campo-definido/campo-definido.entity';
...
export class TipoDeterioroAnalisisCriticidadConfigService {
  ...
  constructor(
    @InjectRepository(TipoDeterioroAnalisisCriticidadConfig)
    private tipoDeterioroAnalisisCriticidadRepository: Repository<TipoDeterioroAnalisisCriticidadConfig>,
  ...
  public async getAllTiposDeteriorosAnalisisCriticidadConfig(
    idMaterialConfig?: number,
    nombre?: String,
    withCamposAfectados: boolean = false,
  ): Promise<
    ApiPaginatedResponse<TipoDeterioroAnalisisCriticidadConfigSerializable[]>
  > {
    const tiposDeteriorosAnalisisCriticidadConfigSerializables: Array<TipoDeterioroAnalisisCriticidadConfigSerializable> =
      new Array<TipoDeterioroAnalisisCriticidadConfigSerializable>();
    // Se obtienen los tipos de deterioro de la base de datos
    const tiposDeteriorosAnalisisCriticidadConfig: Array<TipoDeterioroAnalisisCriticidadConfig> =
      await this.tipoDeterioroAnalisisCriticidadRepository.find({
        where: {
    ...
    for (
      let index = 0;
      index < tiposDeteriorosAnalisisCriticidadConfig.length;
      index++
    ) {
      const tipoDeterioroAnalisisCriticidadConfig: TipoDeterioroAnalisisCriticidadConfig =
        tiposDeteriorosAnalisisCriticidadConfig[index];
      tiposDeteriorosAnalisisCriticidadConfigSerializables.push(
        new TipoDeterioroAnalisisCriticidadConfigSerializable(
          tipoDeterioroAnalisisCriticidadConfig.id,
          tipoDeterioroAnalisisCriticidadConfig.nombre,
          await tipoDeterioroAnalisisCriticidadConfig.cantCamposAfectados(),
          await tipoDeterioroAnalisisCriticidadConfig.cantCausas(),
          tipoDeterioroAnalisisCriticidadConfig.detectabilidad,
          withCamposAfectados
            ? await tipoDeterioroAnalisisCriticidadConfig.camposAfectados
            : undefined,
        ),
      );
    }

    return { data: tiposDeteriorosAnalisisCriticidadConfigSerializables };
  }

  // Método para obtener un tipo de deterioro analisis de criticidad

  private async getTipoDeterioroAnalisisCriticidadConfig(
    idTipoDeterioroConfig?: number,
    idMaterialConfig?: number,
    nombre?: String,
  ) {
  ...
  public async getTipoDeterioroAnalisisCriticidadConfigSerializable(
    idTipoDeterioroConfig?: number,
    idMaterialConfig?: number,
    nombre?: String,
  ) {
    ...
    const camposDefinidos: Array<CampoDefinido> =
      await tipoDeterioroAnalisisCriticidadConfig.camposDefinidos;
    // se crea la lista con los campos definidos del tipo de deterioro
    const camposDefinidosTipoDeterioro: Array<CampoDefinidoDTO> =
      new Array<CampoDefinidoDTO>();
    ...
    for (let index = 0; index < camposDefinidos.length; index++) {
      ...
      if (campoDefinido instanceof CampoDefinidoTexto) {
        // si es de tipo texto
        campoDefinidoDTO.constuirDTO(
          campoDefinido.id,
          campoDefinido.nombre,
          campoDefinido.tipo,
        ); // se crea un dto solo con la información del campo definido tipo texto
      } else if (campoDefinido instanceof CampoDefinidoImagen) {
        // si es de tipo imagen
        campoDefinidoDTO.constuirDTO(
          campoDefinido.id,
          campoDefinido.nombre,
          campoDefinido.tipo,
        ); // se crea un dto solo con la información del campo definido tipo imagen
      } else if (campoDefinido instanceof CampoDefinidoNumerico) {
        // si es de tipo numérico
        campoDefinidoDTO.constuirDTO(
          campoDefinido.id,
          campoDefinido.nombre,
          campoDefinido.tipo,
          campoDefinido.inicioIntervalo,
          campoDefinido.finalIntervalo,
          campoDefinido.unidadMedida,
        ); // se crea un dto solo con la información del campo definido tipo númerico
      } else if (campoDefinido instanceof CampoDefinidoSeleccion) {
        // si es de tipo selección
        campoDefinidoDTO.constuirDTO(
          campoDefinido.id,
          campoDefinido.nombre,
          campoDefinido.tipo,
          ...
          undefined,
          undefined,
          JSON.parse(campoDefinido.opciones.toString()),
        ); // se crea un dto solo con la información del campo definido tipo númerico
      }
  ...
  public async createTipoDeterioroAnalisisCriticidadConfig(
    tipoDeterioroAnalisisCriticidadConfigDTO: TipoDeterioroConfigDTO,
    entityManager?: EntityManager,
  ) {
    // Si no existe un tipo de deterioro con el mismo nombre
    if (
      !(await this.getTipoDeterioroAnalisisCriticidadConfig(
        undefined,
        tipoDeterioroAnalisisCriticidadConfigDTO.materialConfig.id,
        tipoDeterioroAnalisisCriticidadConfigDTO.nombre,
      ))
    ) {
      if (!entityManager)
        // No se trata de una llamada con una transacción heredada
        await this.tipoDeterioroAnalisisCriticidadRepository.manager.transaction(
          async (transactionManager: EntityManager) => {
            // Se crea una transaccion para este procedimiento
            await this.createTipoDeterioroAnalisisCriticidadConfigWithEntityManager(
              tipoDeterioroAnalisisCriticidadConfigDTO,
              transactionManager,
            );
          },
        );
      ...
      throw new HttpException(
        'Ya existe un tipo de deterioro con ese nombre',
        HttpStatus.BAD_REQUEST,
      );
  }

  private async createTipoDeterioroAnalisisCriticidadConfigWithEntityManager(
    tipoDeterioroAnalisisCriticidadConfigDTO: TipoDeterioroConfigDTO,
    entityManager: EntityManager,
  ) {
    const tipoDeterioroAnalisisCriticidadConfig: TipoDeterioroAnalisisCriticidadConfig =
      new TipoDeterioroAnalisisCriticidadConfig(
        undefined,
        tipoDeterioroAnalisisCriticidadConfigDTO.nombre,
        tipoDeterioroAnalisisCriticidadConfigDTO.tipo,
        tipoDeterioroAnalisisCriticidadConfigDTO.materialConfig instanceof
        MaterialConfig
          ? tipoDeterioroAnalisisCriticidadConfigDTO.materialConfig
          : new MaterialConfig(
              tipoDeterioroAnalisisCriticidadConfigDTO.materialConfig.id,
            ),
        tipoDeterioroAnalisisCriticidadConfigDTO.detectabilidad,
        new Array<Campo>(),
      ); /* se crea
    ...
    if (tipoDeterioroAnalisisCriticidadConfigDTO.camposAfectados)
      await this.saveCamposInTipoDeterioroAnalisisCriticidad(
        tipoDeterioroAnalisisCriticidadConfigDTO.camposAfectados,
        tipoDeterioroAnalisisCriticidadConfig,
        entityManager,
      ); /*Se
         insertan los campos pertenecientes al tipo de deterioro analisis de criticidad antes de insertarlo en la base de datos */

    const tipoDeterioroAnalisisCriticidadConfigInsertado: TipoDeterioroAnalisisCriticidadConfig =
    ...
    if (tipoDeterioroAnalisisCriticidadConfigDTO.camposDefinidos)
      await this.saveCamposDefinidos(
        tipoDeterioroAnalisisCriticidadConfigDTO.camposDefinidos,
        tipoDeterioroAnalisisCriticidadConfigInsertado,
        entityManager,
      ); // se insertan todos los campos definidos pertenecientes al tipo de deterioro en la base de datos

    if (tipoDeterioroAnalisisCriticidadConfigDTO.causas)
      await this.saveCausas(
        tipoDeterioroAnalisisCriticidadConfigDTO.causas,
        tipoDeterioroAnalisisCriticidadConfigInsertado,
        entityManager,
      ); /*Se insertan las causas
            pertencientes al tipo de deterioro*/
  }

  // Metodo para insertar todos los camposDefinidos pertenecientes al tipo de deterioro
  private async saveCamposDefinidos(
    camposDefinidosDTO: Array<CampoDefinidoDTO>,
    tipoDeterioroAnalisisCriticidadConfigInsertado: TipoDeterioroAnalisisCriticidadConfig,
    entityManager: EntityManager,
  ) {
    ...
    for (let index = 0; index < camposDefinidosDTO.length; index++) {
      camposDefinidosDTO[index].tipoDeterioroConfig =
        tipoDeterioroAnalisisCriticidadConfigInsertado; // Se Registra
      if (camposDefinidosDTO[index].tipo == TiposCamposDefinidos.Numerico)
        // si el campo definido es de tipo numérico
  ...
  private async saveCamposInTipoDeterioroAnalisisCriticidad(
    camposDTO: Array<CampoDTO>,
    tipoDeterioroAnalisiCriticidad: TipoDeterioroAnalisisCriticidadConfig,
    entityManager: EntityManager,
  ) {
      ...
      for (let index = 0; index < camposDTO.length; index++) {
        const campo: Campo | undefined = await entityManager.findOne(Campo, {
          where: {
            ...
            configVersion: camposDTO[index].configVersion
              ? camposDTO[index].configVersion
              : tipoDeterioroAnalisiCriticidad.materialConfig.subsistemaConfig
                  .sistemaConfig.config.version,
          },
        }); // se obtiene el campo perteneciente a la herramienta

        if (campo) {
          // si fue encontrado el campo

          camposAfectadosTipoDeterioro.push(campo); // se añade el campo como parte del campo del tipoDeterioro analisis criticidad
        } else throw new HttpException('No se encontró campo', 401);
      }
    } else {
      // si si se le fue proporcionada una id
      for (let index = 0; index < camposDTO.length; index++) {
        ...
        if (campo)
          ...
          throw new HttpException(
            'El campo proporcionado no se encuentra en la base de datos',
            HttpStatus.BAD_REQUEST,
          );
      }
    }
    ...
    tipoDeterioroAnalisiCriticidad.camposAfectados = Promise.resolve(
      camposAfectadosTipoDeterioro,
    );
  }

  // Metodo para insertar todas la causas del tipo de deterioro
  private async saveCausas(
    causasDTO: Array<CausaDTO>,
    tipoDeterioroAnalisisCriticidadConfigInsertado: TipoDeterioroAnalisisCriticidadConfig,
    entityManager: EntityManager,
  ) {
    ...
    for (let index = 0; index < causasDTO.length; index++) {
      causasDTO[index].tipoDeterioroConfig =
        tipoDeterioroAnalisisCriticidadConfigInsertado; // Se Registra
  ...
  // IMPLEMENTAR
  public async updateTipoDeterioroConfigAnalisisCriticidadConfig(
    idTipoDeterioroAnalisisCriticidadConfig: number,
    ...
    const tipoDeterioroAnalisisCriticidadConfigUpdate: TipoDeterioroAnalisisCriticidadConfig =
      await this.getTipoDeterioroAnalisisCriticidadConfig(
        idTipoDeterioroAnalisisCriticidadConfig,
      );
    // se busca un tipo de deterioro que posea el mismo nombre que el nuevo nombre del tipo de deterioro
    const tipoDeterioroAnalisisCriticidadConfig: TipoDeterioroAnalisisCriticidadConfig =
      await this.getTipoDeterioroAnalisisCriticidadConfig(
        undefined,
        tipoDeterioroAnalisisCriticidadConfigUpdate.materialConfigId,
    ...
    await this.tipoDeterioroAnalisisCriticidadRepository.manager.transaction(
      async (transactionManager: EntityManager) => {
        ...
        if (
          ...
          tipoDeterioroAnalisisCriticidadConfigUpdate.nombre =
            updateTipoDeterioroConfigAnalisisCriticidadConfigDTO.nombre;
          // Se actualiza la detectabilidad del registro
          tipoDeterioroAnalisisCriticidadConfigUpdate.detectabilidad =
            updateTipoDeterioroConfigAnalisisCriticidadConfigDTO.detectabilidad;
          // Se actualizan sus relaciones
          // Se actualizan las causas registradas
          await this.actualizarCausasTipoDeterioroAnalisisCriticidad(
            tipoDeterioroAnalisisCriticidadConfigUpdate,
            updateTipoDeterioroConfigAnalisisCriticidadConfigDTO.causas,
            transactionManager,
          );
          // Se actualizan los campos definidos registrados
          await this.actualizarCamposDefinidosTipoDeterioroAnalisisCriticidad(
            tipoDeterioroAnalisisCriticidadConfigUpdate,
            updateTipoDeterioroConfigAnalisisCriticidadConfigDTO.camposDefinidos,
            transactionManager,
          );
          // Se actualizan los campos afectados registrados como parte de la relación de mucho a mucho
          await this.actualizarCamposAfectadosTipoDeterioroAnalisisCriticidad(
            tipoDeterioroAnalisisCriticidadConfigUpdate,
            updateTipoDeterioroConfigAnalisisCriticidadConfigDTO.camposAfectados,
            transactionManager,
          );
  ...
  private async actualizarCausasTipoDeterioroAnalisisCriticidad(
    tipoDeterioroAnalisisCriticidadUpdate: TipoDeterioroAnalisisCriticidadConfig,
    causas: Array<CausaDTO>,
    entityManager: EntityManager,
  ) {
    // Lo primero es eliminar todas las causas pertenecientes al tipo de deterioro análisis criticdad
    await this.causasService.deleteCausas(
      tipoDeterioroAnalisisCriticidadUpdate.id,
    );
    // Luego se insertan las nuevas causas del tipo de deterioro
    await this.saveCausas(
      causas,
      tipoDeterioroAnalisisCriticidadUpdate,
      entityManager,
    );
  }

  private async actualizarCamposDefinidosTipoDeterioroAnalisisCriticidad(
    tipoDeterioroAnalisisCriticidadUpdate: TipoDeterioroAnalisisCriticidadConfig,
    ...
    await this.campoDefinidoService.deleteCamposDefinidos(
      tipoDeterioroAnalisisCriticidadUpdate.id,
      entityManager,
    );
  ...
  private async actualizarCamposAfectadosTipoDeterioroAnalisisCriticidad(
    tipoDeterioroAnalisisCriticidadUpdate: TipoDeterioroAnalisisCriticidadConfig,
  ...
  public async getCampos(tipoDeterioroAnalisisCriticidadId: number) {
    ...
    const tipoDeterioroAnalisiCriticidad =
      await this.tipoDeterioroAnalisisCriticidadRepository.findOne({
        where: {
    ...
    if (tipoDeterioroAnalisiCriticidad)
      ...
      throw new HttpException(
        'Ya existe un tipo de deterioro con ese identificador',
        HttpStatus.BAD_REQUEST,
      );
  }
}
