...
export class SistemasConfigService {
  constructor(
    ...
    private sistemaConfigRepository: Repository<SistemaConfig>,
  ...
  public async createSistemaConfig(
    sistemaConfigDTO: SistemaConfigDTO,
    entityManager?: EntityManager,
  ) {
    // si no existe un sistema con ese nombre
    if (
      !(await this.getSistemaConfig(
        undefined,
        sistemaConfigDTO.nombre,
        sistemaConfigDTO.config.version,
      ))
    ) {
      // si no existe el sistema config
      if (!entityManager)
        // No se trata de una llamada con una transacción heredada
        await this.sistemaConfigRepository.manager.transaction(
          async (transactionManager: EntityManager) => {
            // Se crea una transaccion para este procedimiento
            await this.createSistemaConfigWithEntityManager(
              sistemaConfigDTO,
              transactionManager,
            );
          },
      ...
      throw new HttpException(
        'Ya existe un sistema con el mismo nombre',
  ...
  public async updateSistemaConfigDTO(
    idSistemaConfig: number,
    updateSistemaConfigDTO: UpdateSistemaConfigDTO /* representa los datos a actualizar del sistema config */,
  ) {
    const sistemaConfig: SistemaConfig = await this.getSistemaConfig(
      undefined /*se indica que no se va a filtar por este campo */,
      updateSistemaConfigDTO.nombre,
      updateSistemaConfigDTO.configVersion,
    );

    if (!sistemaConfig || sistemaConfig.id === idSistemaConfig) {
      // si no existe un configuración con el mismo nombre o si la que existe es ella misma (significa esto ultimo que el usuario  cambió el nombre de ese sistema config por otro)
      const sistemaConfigUpdate: SistemaConfig =
        await this.getSistemaConfig(idSistemaConfig); // se obtiene el sistema config a modificar
      sistemaConfigUpdate.nombre = updateSistemaConfigDTO.nombre; // se actualiza el nombre
      const herramienta: Herramienta =
        await this.herramientaService.getHerramientaById(
          updateSistemaConfigDTO.herramienta.id,
        ); // se obtiene la herramienta

      if (herramienta) {
        // si fue encontrada herramienta con ese id
  ...
  public async getAllSistemasConfig(
  ...
  ): Promise<ApiPaginatedResponse<SistemaConfigSerializable[]>> {
    const sistemasConfigSerializable: Array<SistemaConfigSerializable> =
      new Array<SistemaConfigSerializable>();
    // se obtienen los sistemas config que cumplen con los valores de los filtros
    const sistemasConfig: Array<SistemaConfig> =
      await this.sistemaConfigRepository.find({
        // filtro de config
        where: {
    ...
    if (sistemasConfig) {
      for (let index = 0; index < sistemasConfig.length; index++) {
        ...
        const sistemaConfigSerializable: SistemaConfigSerializable =
          new SistemaConfigSerializable(
            sistemaConfig.id,
            sistemaConfig.nombre,
            await sistemaConfig.cantSubsistemasConfig(),
            await sistemaConfig.getHerramienta(),
            sistemaConfig.configVersion,
          ); // se construye el sistema config serializable
        if (
          (nombreHerramienta &&
            sistemaConfigSerializable.herramienta.nombre
              .toLocaleLowerCase()
              .includes(nombreHerramienta.toString().toLocaleLowerCase())) ||
          !nombreHerramienta
        )
  ...
  private async createSistemaConfigWithEntityManager(
    ...
    if (sistemaConfigDTO.herramienta.id)
      // si se proporcionó  un id para a herramienta

      herramientaAsignada = await entityManager.findOne(Herramienta, {
        // se busca una herramienta por ese id y por la configuracion
        where: {
      ...
      // si no se proporcionó un id para la herramienta

      herramientaAsignada = await entityManager.findOne(Herramienta, {
        // se busca una herramienta en la base de datos para la misma configuración que tenga el mismo nombre
        where: {
    ...
    if (herramientaAsignada) {
      // si fue posible encontrar una herramienta

      // Si no existe un sistema config con el mismo nombre en esa configuración

      // Se crea el sistemaConfig
      const sistemaConfig: SistemaConfig = new SistemaConfig(
        undefined,
        sistemaConfigDTO.nombre,
        herramientaAsignada,
        sistemaConfigDTO.config instanceof Config
          ? sistemaConfigDTO.config
          : new Config(sistemaConfigDTO.config.version),
      ); // se crea un sistema config listo para ser insertado

      const sistemaConfigInsertado: SistemaConfig =
        await entityManager.save(sistemaConfig); // se inserta en la base de datos el sistema de configuracion, y se obtiene la instancia  insertada con un id

      if (sistemaConfigDTO.subSistemasConfig)
        await this.saveSubSistemasConfig(
          sistemaConfigDTO.subSistemasConfig,
          sistemaConfigInsertado,
          entityManager,
        ); // se inserta en la base de datos los subSistemasConfig pertenecientes a este sistemaConfig
    } else
      throw new HttpException(
        'Herramienta no encontrada dentro de la configuración',
        HttpStatus.BAD_REQUEST,
      );
  }

  // Metodo para insertar todos los subSistemasConfigs pertenecientes al sistemaConfig
  private async saveSubSistemasConfig(
    subSistemasConfigDTO: Array<SubsistemaConfigDTO>,
    sistemaConfigInsertado: SistemaConfig,
    entityManager: EntityManager,
  ) {
    ...
    for (let index = 0; index < subSistemasConfigDTO.length; index++) {
      ...
      await this.subSistemaConfigService.createSubSistemaConfig(
        subSistemasConfigDTO[index],
        entityManager,
      ); // Se manda a insertar al servicio de subSistemaConfig en la base de datos
    }
  }
  ...
  public async deleteSistemaConfig(idSistemaConfig: number) {
    ...
    const sistemaConfigEliminar: SistemaConfig | undefined =
      await this.sistemaConfigRepository.findOne({
        where: {
          id: idSistemaConfig,
        },
      });
    ...
    if (sistemaConfigEliminar) {
      ...
      await this.eventEmitter.emitAsync(
        'accionCritica',
        sistemaConfigEliminar.configVersion,
      );
    } else
      throw new HttpException(
        'No existe un Sistema Config con ese id',
        HttpStatus.BAD_REQUEST,
      );
  }
  ...
  public async getHerramientaSistemaMaterial(
    ...
    for (
      let index = 0;
      index < sistemasConfig.length && !herramienta;
      index++
    ) {
      const sistemaConfig: SistemaConfig = sistemasConfig[index];
      if (await sistemaConfig.isContainsMaterial(idMaterial))
        // si el sistema config contiene al material
